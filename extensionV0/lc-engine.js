    ...
    for (let i = 0; i < active.length; i++) {
        for (let j = i + 1; j < active.length; j++) {
            ...
            if (distH >= distV) {
                ...
                if (yOverlap <= 0) { console.log(`par (${a.index + 1},${b.index + 1}) H: pulado yOverlap=${yOverlap.toFixed(4)}`); continue; }
                ...
                console.log(`par (${left.index + 1},${right.index + 1}) H: currentGap=${currentGap.toFixed(4)} target=${gapH.toFixed(4)} diff=${diff.toFixed(4)}`);

                if (Math.abs(diff) > 0.001) {
                    const half = diff / 2;
                    const snapLW = left.w, snapRX = right.x, snapRW = right.w;
                    ...
                    const enforced = (left.w !== +(snapLW + half).toFixed(6) || right.w !== +(snapRW + half).toFixed(6));
                    console.log(`  → left.w ${snapLW.toFixed(4)}→${left.w.toFixed(4)}, right.x ${snapRX.toFixed(4)}→${right.x.toFixed(4)}, right.w ${snapRW.toFixed(4)}→${right.w.toFixed(4)}${enforced ? ' [enforce travou]' : ''}`);
                    changedCount++;
                }
            } else {
                // Vertical pair — skip if Y is locked
                if (lc.gapLockY) { console.log(`par (${a.index + 1},${b.index + 1}) V: pulado gapLockY`); continue; }
                const xOverlap = Math.min(a.x + a.w, b.x + b.w) - Math.max(a.x, b.x);
                if (xOverlap <= 0) { console.log(`par (${a.index + 1},${b.index + 1}) V: pulado xOverlap=${xOverlap.toFixed(4)}`); continue; }
                ...
                console.log(`par (${top.index + 1},${bot.index + 1}) V: currentGap=${currentGap.toFixed(4)} target=${gapV.toFixed(4)} diff=${diff.toFixed(4)}`);
